...
class Profil extends CI_Controller
{
    ...
    public function keranjang($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert"><i class="icon fas fa-check-circle"></i>Data Obat Berhasil Ditambahkan ke keranjang!</div>');
    ...
    public function delkeranjang($id)
    {
        ...
        $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert"><i class="icon fas fa-check-circle"></i>Data Berhasil Dihapus dari keranjang!</div>');
        redirect('Profil/detail');
    }
    ...
    public function pesanan()
    {
        ...
        if ($this->Penjualan_model->insert($data_p, $upload_image) && $this->Detail_model->insert($data_detail)) {
            for ($i = 0; $i < $jumlah_beli; $i++) {
                $this->Obat_model->min_stok($data_detail[$i]['jumlah'], $data_detail[$i]['id_obat']) or die('gagat min stok');
            }
            $id_us = $this->session->userdata('id');
            $this->Keranjang_model->delete_all($id_us);
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert"><i class="icon fas fa-check-circle"></i>Pesanan Berhasil dibuat!</div>');
            redirect('Profil/obat');
        } else {
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert"><i class="icon fas fa-info-circle"></i>Pesanan Gagal dibuat!</div>');
            redirect('Profil/obat');
        }
    }
    ...
    public function statusbeli($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert"> <i class="icon fas fa-check-circle"></i>Status Berhasil di Ubah!</div>');
